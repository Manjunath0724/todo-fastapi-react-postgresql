  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav className={`p-4 space-y-2 flex-grow overflow-y-auto overflow-x-hidden ${!isOpen ? 'flex flex-col items-center' : ''}`}>
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
            ...
            >
              <item.icon className={`w-6 h-6 flex-shrink-0 transition-transform group-hover:scale-110 ${!isOpen ? '' : ''}`} />
              {isOpen && (
        ...
        <button
          ...
          className="absolute -right-4 top-24 w-8 h-8 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors z-50 hidden lg:flex"
        >
          {isOpen ? <X size={14} /> : <Menu size={14} />}
        </button>

        {/* User Footer */}
        <div className={`absolute bottom-6 left-0 right-0 px-4 ${!isOpen ? 'flex justify-center' : ''}`}>
          {isOpen ? (
